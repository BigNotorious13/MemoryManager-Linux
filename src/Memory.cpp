...
namespace Memory {
    ...
    uintptr_t FindDMAAddy(Handle hProc, uintptr_t ptr, std::vector<unsigned int> offsets){
        ...
        for(auto offset : offsets){
            hProc.Read((void *)addr, &addr, sizeof(addr));
            addr += offset;
        }
        return addr;
    }
    ...
    bool Handle::Write(void* address, void* buffer, size_t size) {
        ...
        struct iovec Memory[1];

        local[0].iov_base = buffer;
        local[0].iov_len = size;
        Memory[0].iov_base = address;
        Memory[0].iov_len = size;

        return (process_vm_writev(pid, local, 1, Memory, 1, 0) == size);
    }
    ...
    bool Handle::Read(void* address, void* buffer, size_t size) {
        ...
        return (process_vm_readv(pid, local, 1, Memory, 1, 0) == size);
    }
...
// Functions Exported
bool Memory::FindProcessByName(std::string name, Memory::Handle* out) {
    ...
    if (dir) {
        while ((dire = readdir(dir)) != NULL) {
            ...
            Memory::Handle proc(dire->d_name);
